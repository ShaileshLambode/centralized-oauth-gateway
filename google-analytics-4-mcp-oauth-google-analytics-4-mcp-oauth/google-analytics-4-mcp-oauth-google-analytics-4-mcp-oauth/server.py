...
def get_effective_session_id(session_id: str, ctx: Context = None) -> str:
    """Helper to resolve session ID from context if default is provided."""
    if session_id != "default":
        return session_id
    
    if ctx:
        try:
             # Try to access session_id from Context properties
             return ctx.session_id
        except Exception as e:
             # Fallback if property access fails or context not ready
             logger.debug(f"Could not get session_id from context: {e}")
             pass
    
    return "default"

async def _resolve_auth_headers(session_id: str, ctx: Context = None) -> Dict[str, str]:
    """Helper to resolve session and get headers for requests."""
    effective_session_id = get_effective_session_id(session_id, ctx)
    target_session_id = resolve_session_id(effective_session_id)
    return get_headers_with_auto_token(target_session_id)
...
async def list_properties(
    ...
    Args:
        ...
        session_id: Session ID for multi-user support (default: 'default')
    ...
    try:
        # Resolve session and get headers
        headers = await _resolve_auth_headers(session_id, ctx)
        ...
        async with httpx.AsyncClient() as client:
            ...
            if account_id:
                ...
                if account_response.status_code != 200:
                    if ctx:
                        ctx.error(f"Failed to get account {account_id}: {account_response.status_code} {account_response.reason_phrase}")
                    raise Exception(f"Admin API error: {account_response.status_code} {account_response.reason_phrase} - {account_response.text}")
                ...
                while True:
                    ...
                    try:
                        ...
                        if properties_response.status_code == 200:
                            ...
                            alt_properties_url = f"https://analyticsadmin.googleapis.com/{api_version}/properties"
                            # Filter needs to be in query params
                            alt_params = {'filter': f"parent:{account_name}"}
                            if properties_next_token:
                                alt_params['pageToken'] = properties_next_token
                                
                            alt_response = await client.get(alt_properties_url, headers=headers, params=alt_params)
                            
                            if alt_response.status_code == 200:
                                alt_results = alt_response.json()
                                properties.extend(alt_results.get('properties', []))
                                properties_next_token = alt_results.get('nextPageToken')
                            else:
                                if ctx:
                                    ctx.error(f"Failed to get properties: {properties_response.status_code} {properties_response.reason_phrase}")
                                raise Exception(f"Admin API error: {properties_response.status_code} {properties_response.reason_phrase} - {properties_response.text}")
                ...
                # Check API version availability and fetch accounts
                accounts_url = 'https://analyticsadmin.googleapis.com/v1/accounts'
                
                # Initial check to see if v1 works
                check_response = await client.get(accounts_url, headers=headers, params={'pageSize': 1})
                ...
                while True:
                    ...
                    if accounts_response.status_code != 200:
                        if ctx:
                            ctx.error(f"Failed to list accounts: {accounts_response.status_code} {accounts_response.reason_phrase}")
                        raise Exception(f"Admin API error: {accounts_response.status_code} {accounts_response.reason_phrase} - {accounts_response.text}")

                    accounts_results = accounts_response.json()
                    accounts.extend(accounts_results.get('accounts', []))
                    next_page_token = accounts_results.get('nextPageToken')
        ...
        # Authentication required - return auth URL
        error_msg = str(e)
        if "AUTHENTICATION REQUIRED" in error_msg:
            return {"error": "Authentication required", "message": error_msg, "type": "auth_required"}
        raise
...
async def get_page_views(
    ...
    try:
        ...
        if response.status_code != 200:
            if ctx:
                ctx.error(f"Google Analytics API error: {response.status_code} {response.reason_phrase}")
            raise Exception(f"Google Analytics API error: {response.status_code} {response.reason_phrase} - {response.text}")
...
if __name__ == "__main__":
    ...
    if "--http" in sys.argv:
        logger.info("Starting with HTTP transport (uvicorn) on http://0.0.0.0:8000")
        logger.info("MCP endpoint: http://0.0.0.0:8000/mcp")
        logger.info("OAuth callback: http://0.0.0.0:8000/callback")
        ...
        logger.info("✓ Registered OAuth callback route at /callback")
